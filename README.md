...
| [docs/PLATFORM_LAYER.md](docs/PLATFORM_LAYER.md) | Design for the Platform Layer / `CommandRunner`, centralizing OS process execution in `cli/` (see [ADR-0009](docs/decisions/0009-platform-layer-command-runner.md), Accepted; core and `RuntimeContext` wiring implemented) |
| [docs/EFI_ADAPTER.md](docs/EFI_ADAPTER.md) | Design for the EFI Adapter (`bcs.platform.adapters.efi`), a read-only firmware boot configuration wrapper and the first Host Discovery adapter (see [ADR-0010](docs/decisions/0010-efi-adapter-read-only-scope.md), Accepted; fully implemented) |
| [docs/STORAGE_ADAPTER.md](docs/STORAGE_ADAPTER.md) | Design for the Storage Adapter (`bcs.platform.adapters.storage`), a read-only block/partition/filesystem topology wrapper and the second Host Discovery adapter; fully implemented |
| [docs/SECURE_BOOT_ADAPTER.md](docs/SECURE_BOOT_ADAPTER.md) | Design for the Secure Boot Adapter (`bcs.platform.adapters.secureboot`), a read-only firmware Secure Boot state wrapper and the third Host Discovery adapter; domain models, parser, and error hierarchy implemented, adapter execution layer not yet implemented |
| [docs/HOST_DISCOVERY_ORCHESTRATOR.md](docs/HOST_DISCOVERY_ORCHESTRATOR.md) | Design for the Host Discovery Orchestrator, coordinating every Host Discovery adapter into one snapshot (see [ADR-0011](docs/decisions/0011-host-discovery-orchestrator.md), Accepted; implemented, including `RuntimeContext`/composition-root wiring) |
