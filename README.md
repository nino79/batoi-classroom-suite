...
BCS is currently in its **architecture and specification phase**. The goal of this phase is to agree on scope, boundaries, interfaces, and non-functional requirements *before* writing implementation code, so that the three components can be built independently without integration surprises.
