...
- **Each individual adapter's command contract and output model** (`lsblk`, `blkid`, `mount`, `rsync`) is explicitly not designed by this document — only *that* they will exist, where they will live, and how they will depend on `CommandRunner`. The EFI adapter is the exception, now fully designed separately — see [docs/EFI_ADAPTER.md](EFI_ADAPTER.md) and [ADR-0010](decisions/0010-efi-adapter-read-only-scope.md).
