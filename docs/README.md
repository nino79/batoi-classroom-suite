...
| [RELEASE_ENGINEERING.md](RELEASE_ENGINEERING.md) | Standardised release tooling for building, verifying, and publishing Beta releases — `scripts/release/` design doc with workflow, checklist, and rollback procedure. |
...
| [HARDWARE_VALIDATION_TOOLKIT.md](HARDWARE_VALIDATION_TOOLKIT.md) | Reusable Bash scripts for capturing, comparing, and summarizing hardware topology during Beta validation on physical classroom computers — `scripts/hardware-validation/` design doc. |
...
| [TESTING_INFRASTRUCTURE.md](TESTING_INFRASTRUCTURE.md) | The shared `FakeCommandRunner` test double, migration guide, and LOC-reduction metrics for Platform Layer adapter tests. |
| [BETA_VALIDATION_AUTOMATION.md](BETA_VALIDATION_AUTOMATION.md) | Automated Beta validation framework — `scripts/validate-beta.sh`, environment capture, report generation, CI integration, and future extensions. |
| [BETA_DASHBOARD.md](BETA_DASHBOARD.md) | Beta Diagnostics Dashboard — aggregating hardware, validation, release, and packaging results into a single PASS/WARN/FAIL decision report. |
| [BETA_READINESS_AUDIT.md](BETA_READINESS_AUDIT.md) | Comprehensive Beta readiness audit — documentation structure, scripts/tooling, naming consistency, GitHub features mapping, docs technical debt register, reorganisation proposal, and release readiness assessment. |
| [BUILDER_PIPELINE.md](BUILDER_PIPELINE.md) | The 8-stage Builder pipeline — validate config, prepare workspace, resolve assets, generate manifests, generate metadata, copy boot resources, generate artifacts, cleanup — with stage contracts, exit codes, and invocation contract. |
...
| [COLLECTOR_USAGE_CENSUS.md](COLLECTOR_USAGE_CENSUS.md) | Complete inventory of every legacy `collect_*` function: callers, adapter equivalents, migration status, and removal horizon — prepared for the Beta-phase collector deprecation arc (issue #78). |
| [COLLECTOR_CALL_GRAPH.md](COLLECTOR_CALL_GRAPH.md) | Every remaining `collect_*` call site across production code, classified by whether it must disappear, stay as fallback, or stay permanently. |
| [UNUSED_PLATFORM_ADAPTERS.md](UNUSED_PLATFORM_ADAPTERS.md) | Detection and status of every Platform Adapter whose output does not reach a CLI command — EFI, Filesystem, and partial Secure Boot data tracked with consumption gaps and recommendations. |
| [TECHNICAL_DEBT_INDEX.md](TECHNICAL_DEBT_INDEX.md) | Index of every `TODO`/`FIXME`/`placeholder`/`deprecated`/`not implemented`/`pending` marker in `cli/src/` and `docs/`, classified by severity with exact file and line references. |
