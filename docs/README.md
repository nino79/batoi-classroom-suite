...
| [SECURE_BOOT_ADAPTER.md](SECURE_BOOT_ADAPTER.md) | Design (Accepted; domain models, parser, and error hierarchy implemented, adapter execution layer not yet implemented) for the Secure Boot Adapter (`bcs.platform.adapters.secureboot`) — a read-only firmware Secure Boot state wrapper, the third Host Discovery adapter. |
