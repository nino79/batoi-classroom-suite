...
- **Implemented.** `collect_efi_system_partition()` and `collect_usb_storage()` (`cli/src/bcs/inventory/collectors.py`), wired into `collect_host_inventory()` and `bcs inventory`'s text/JSON/YAML rendering, plus the `esp` and `usb-storage` `bcs doctor` checks (`cli/src/bcs/commands/doctor.py`) — see [docs/HOST_INVENTORY.md § Current Implementation Status](../HOST_INVENTORY.md#current-implementation-status-vs-this-proposal).
