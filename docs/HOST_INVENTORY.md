...
| Service orchestration | ✅ Implemented | `collect_host_inventory()` calls all ten collectors when no `HostDiscoveryOrchestrator` is supplied; see the naming note below. When called with one (as `bcs inventory` now does, per issue [#70](https://github.com/nino79/batoi-classroom-suite/issues/70)), `cpu`/`memory`/`network`/`storage` are instead sourced from the Discovery snapshot, falling back to the collector per-field when a slot is unwired or its adapter raised a `PlatformError`; `firmware.secure_boot` follows the identical pattern as one sub-field override (Beta M4) — see [docs/HOST_DISCOVERY_ORCHESTRATOR.md § Relationship to Host Inventory](HOST_DISCOVERY_ORCHESTRATOR.md#relationship-to-host-inventory---implemented). |
...
- **Resolved — IP address enumeration**: `NetworkInterface.ip_addresses` was always `[]` because pure-stdlib per-interface IP discovery isn't portable without a new dependency or shelling out to `ip`/`ifconfig`. Beta M3 closed this gap: `bcs inventory` now sources `network` from the Network Adapter (`ip -json addr show`, wrapped by `bcs.platform.adapters.network`) via the Host Discovery Orchestrator when available, falling back to the original stdlib-only collector otherwise - see [docs/HOST_DISCOVERY_ORCHESTRATOR.md § Relationship to Host Inventory](HOST_DISCOVERY_ORCHESTRATOR.md#relationship-to-host-inventory---implemented) point 5b.
- **Resolved — Secure Boot byte-value parsing**: `FirmwareInfo.secure_boot` used to return `unknown` for any UEFI system with `efivars` present, rather than actually reading the `SecureBoot-<GUID>` variable's value (a placeholder recorded in the legacy collector's own docstring). Beta M4 closed this gap - not by teaching the legacy collector to parse the raw EFI variable bytes itself (the direct-`efivarfs` approach this bullet originally anticipated), but via the already-implemented, `mokutil`-wrapping Secure Boot Adapter: `bcs inventory` sources `firmware.secure_boot` from it via the Host Discovery Orchestrator when available (falling back to the legacy placeholder otherwise), and `bcs doctor`'s Secure Boot check calls the same adapter directly. See [docs/HOST_DISCOVERY_ORCHESTRATOR.md § Relationship to Host Inventory](HOST_DISCOVERY_ORCHESTRATOR.md#relationship-to-host-inventory---implemented) point 5c and [docs/SECURE_BOOT_IMPLEMENTATION_PLAN.md](SECURE_BOOT_IMPLEMENTATION_PLAN.md).
