...
> **Status: Accepted; fully implemented.** The architecture below (as amended during review — see [§ Amendments Incorporated](#amendments-incorporated-during-review)) is approved. **Implemented:** `BootEntry`/`FirmwareBootConfiguration` (`cli/src/bcs/platform/adapters/efi/models.py`), `parse_firmware_boot_configuration` (`cli/src/bcs/platform/adapters/efi/parser.py`), `read_firmware_boot_configuration` (`cli/src/bcs/platform/adapters/efi/adapter.py`), and `FirmwareBootError`/`FirmwareBootUnavailableError`/`FirmwareBootParseError` (`cli/src/bcs/platform/adapters/efi/errors.py`) — the complete adapter as designed in this document.
...
└── efi/                          # the EFI domain - see naming note below.
    ...
    ├── __init__.py                  # [implemented] re-exports FirmwareBootConfiguration,
    │                              # BootEntry, parse_firmware_boot_configuration,
    │                              # read_firmware_boot_configuration, FirmwareBootError,
    │                              # FirmwareBootUnavailableError, FirmwareBootParseError
    ...
    ├── adapter.py                   # [implemented] read_firmware_boot_configuration(runner: CommandRunner) ->
    │                              # FirmwareBootConfiguration - the only place this package
    │                              # calls CommandRunner.run(), and the only place that knows
    │                              # the backend is efibootmgr
    └── errors.py                    # [implemented] FirmwareBootError(PlatformError) and its two subclasses
```
...
| `adapter.read_firmware_boot_configuration` **(implemented)** | Correct command (`["efibootmgr", "-v"]`), correct locale-forced `env`, correct explicit `timeout_seconds`, `check=False`, and that a successful result is handed off correctly to the parser. | `FakeCommandRunner` programmed to return a `CommandResult` wrapping one of the same fixture texts as `stdout` — see `cli/tests/test_platform_adapters_efi_adapter.py`. |
| Error mapping **(implemented)** | Each condition in [§ Error Mapping](#error-mapping) maps to the right exception. | `FakeCommandRunner` programmed to return/raise the corresponding failure shape (a non-zero `CommandResult` with "not supported"-style `stderr`; a zero-exit `CommandResult` with garbage `stdout`; letting `CommandNotFoundError`/`CommandTimeoutError` pass through unchanged from a fake that raises them directly) — see `cli/tests/test_platform_adapters_efi_adapter.py`. |
...
- **What exposes this to a `bcs` command at all?** Resolved at the coordination layer: the Host Discovery Orchestrator (`docs/HOST_DISCOVERY_ORCHESTRATOR.md`, `Accepted`, implemented) wires this adapter into its `efi` slot, and `collect_host_inventory()` accepts an optional `orchestrator` parameter. What remains open is narrower — whether `FirmwareBootConfiguration` becomes a new `bcs.inventory.models.HostInventory` field, and which `bcs` command, if any, actually passes `runtime.host_discovery_orchestrator` through (none does yet) — see [docs/HOST_DISCOVERY_ORCHESTRATOR.md § Relationship to Host Inventory](HOST_DISCOVERY_ORCHESTRATOR.md#relationship-to-host-inventory---implemented).
