...
`bcs inventory` is backed by a dedicated subsystem (`bcs.inventory` in the implementation — see [cli/README.md](../cli/README.md)), not just another command's private logic, because its output is meant to be **the single source of truth describing the current machine**, consumed identically by four different things:

1. `bcs doctor` itself — its `firmware`, `secure-boot`, `storage`, `network`, and `tooling` checks evaluate pass/fail *against* Host Inventory facts rather than probing the host a second time, so `doctor` and `inventory` can never disagree about what the machine looks like.
2. Boot Manager, Builder, and Deploy (once implemented, in Bash per `ADR-0004`) — they read the exact same JSON `bcs inventory --output json` produces; there is no second, Bash-specific probing implementation to keep in sync.
3. A future REST API — the model is plain, versioned, JSON-serializable data with no CLI dependency, so serving it over HTTP is a transport change, not a redesign.
4. A future Web UI — same reasoning, one layer further out.

Every model in this subsystem is **immutable** (frozen) and carries its own `schemaVersion` (`bcs-inventory/v1alpha1`), independent of `bcs-cli/v1alpha1` (`bcs`'s own output schema) and `bcs/v1alpha1` (ClassroomConfig) — see [Extensibility & Versioning](#extensibility--versioning). Collectors (one function per fact: firmware, storage, network, identity, OS, CPU, memory, tooling) never print; only the `bcs inventory` command formats output, per `CLI-005`.
