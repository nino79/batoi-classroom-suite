...
> **Status: Accepted; fully implemented.** This document is the authoritative design for the Filesystem Adapter, the fourth Host Discovery adapter in BCS's Platform Layer, following the same ports-and-adapters architecture as the [EFI Adapter](EFI_ADAPTER.md) (`Accepted`, implemented), the [Storage Adapter](STORAGE_ADAPTER.md) (`Accepted`, implemented), and the [Secure Boot Adapter](SECURE_BOOT_ADAPTER.md) (`Accepted`, fully implemented). **Implemented:** `FilesystemUsage`/`FilesystemUsageReport` (`cli/src/bcs/platform/adapters/filesystem/models.py`), per [§ Domain Models](#domain-models); the pure parser, `parse_filesystem_usage` (`cli/src/bcs/platform/adapters/filesystem/parser.py`), per [§ Parser Architecture](#parser-architecture); the error hierarchy (`cli/src/bcs/platform/adapters/filesystem/errors.py`), per [§ Error Hierarchy](#error-hierarchy); the orchestration adapter, `read_filesystem_usage` (`cli/src/bcs/platform/adapters/filesystem/adapter.py`), per [§ Adapter Responsibilities](#adapter-responsibilities); and `RuntimeContext`/Host Discovery Orchestrator composition-root wiring — the `filesystem` slot in `HostDiscoveryAdapters`/`HostDiscoverySnapshot` is bound in `bcs.app.main()`, sharing the same `CommandRunner` instance as `efi`/`storage`/`secure_boot` (see [docs/HOST_DISCOVERY_ORCHESTRATOR.md § Dependency Injection Strategy](HOST_DISCOVERY_ORCHESTRATOR.md#dependency-injection-strategy---implemented)) — the complete adapter as designed in this document, fully wired. See [§ ADR Recommendation](#adr-recommendation) for why this document concludes no new ADR is required.
...
└── filesystem/                    # the filesystem-usage domain - see naming note below.
    ...
    │                              # FilesystemUsageReport, parse_filesystem_usage,
    │                              # read_filesystem_usage, FilesystemError,
    │                              # FilesystemUnavailableError, FilesystemParseError
    ├── models.py                    # [implemented] FilesystemUsage, FilesystemUsageReport
    │                              # (frozen, JSON-serializable) - see § Domain Models
    ├── parser.py                    # [implemented] parse_filesystem_usage(text: str) ->
    │                              # FilesystemUsageReport - a pure function; see
    │                              # § Parser Architecture for its independence guarantees
    ├── adapter.py                   # [implemented] read_filesystem_usage(runner: CommandRunner) ->
    ...
    └── errors.py                    # [implemented] FilesystemError(PlatformError) and its two subclasses
```
...
| Non-zero exit, `stdout` parses into **zero** entries, `stderr` matches a known "environment cannot provide data" pattern | `errors.FilesystemUnavailableError` | The *semantic* failure — "this environment cannot answer this question" — kept distinct from "the tool itself is broken," mirroring `StorageUnavailableError`/`FirmwareBootUnavailableError`'s own role exactly. Carries `result` (the full `CommandResult`, including `stderr`) — nothing is lost even on this path. **Matching rule:** `FilesystemUnavailableError` is raised when `stderr` (decoded as UTF-8, replacing malformed bytes) matches **any** of the following `df`-specific patterns (case-insensitive prefix match on the first line of `stderr`): `df: '/<path>': Permission denied`, `df: '<path>': Permission denied`, `df: cannot read table of mounted filesystem`, `df: no such file or directory`, `df: transport endpoint is not connected`, `df: stale file handle`, `df: mount point does not exist`. These patterns cover the concrete cases this exception is designed for: permission-denied mounts, unmounted stale references, restricted namespaces, and similar environment-level failures. A `stderr` that does not match any of these patterns falls through to the base `FilesystemError` row below. |
...
| Zero exit, but `stdout` parses into zero entries | `errors.FilesystemParseError` | A zero-entry result with a clean exit — a real, if very unusual, machine state (a minimal container or namespace with no mounts at all could still legitimately produce one, even with `-a` always passed, per [§ Adapter Responsibilities](#adapter-responsibilities), point 1). This is the same exception type used for malformed `df` output; the caller cannot distinguish the two cases from the exception alone. This design accepts that ambiguity rather than resolving it further; see [§ Open Questions](#open-questions). |
...
This document's earlier draft treated a partial `df` failure by returning a normal `FilesystemUsageReport` and stopping there — which is exactly the shape that is invisible to `caveats` now that this adapter is wired into `HostDiscoveryAdapters.filesystem`: the orchestrator sees a clean, uncaveated success, even though a mount was silently dropped. `raw_stderr` ([§ Domain Models](#domain-models)) exists specifically to close that gap **without** requiring a change to the orchestrator (out of scope for an adapter design document, and not proposed here): the fact that something was unreadable survives on the returned object itself, one layer below where `caveats` operates, rather than vanishing before it ever reaches even that boundary.
...
**Consequences of wiring this adapter into `HostDiscoveryAdapters.filesystem`:**
1. A partial `df` failure that returns a `FilesystemUsageReport` with at least one entry does **not** generate an entry in `HostDiscoverySnapshot.caveats` — the orchestrator sees a clean, uncaveated success.
2. The failure signal (e.g., the stale-mount warning) survives only in `FilesystemUsageReport.raw_stderr`, one layer below where `caveats` operates.
3. This is an intentional consequence of the current orchestrator architecture: `_call_adapter` triggers `caveats` exclusively when the wired callable *raises*, and this adapter's design deliberately returns a value instead of raising when `df` produced at least one parsable entry.
4. Changing that behavior — so that a non-zero `df` exit with partial data would surface in `caveats` — is a decision for `HostDiscoveryOrchestrator` and its ADR, not a decision this adapter can make unilaterally.
...
| Composition-root wiring **(implemented)** | The `filesystem` slot's `functools.partial` binding shares the same `CommandRunner` as `efi`/`storage`/`secure_boot`; a `df`-specific `PlatformError` isolates into its own `caveats` entry without affecting other domains; a partial `df` failure (non-zero exit, ≥1 parsed entry) surfaces via `FilesystemUsageReport.raw_stderr`, not a `caveats` entry. | `cli/tests/test_host_discovery_wiring.py` and `cli/tests/test_host_discovery_pipeline.py`, mirroring the equivalent `efi`/`storage`/`secure_boot` coverage — see [docs/HOST_DISCOVERY_ORCHESTRATOR.md § Testing Strategy](HOST_DISCOVERY_ORCHESTRATOR.md#testing-strategy). |

## Fixtures Strategy

The domain category `cli/tests/fixtures/filesystem/` already exists, reserved during the Host Discovery fixtures-infrastructure work. Its README now correctly documents the `df`-backed capture command and the required scenario list below; the scenario files themselves remain zero-byte placeholders — no real capture has been taken yet:
...
- `cli/tests/fixtures/filesystem/README.md` already reflects the above (capture command, flat layout, naming convention, and the six required scenarios as an inventory table). Capturing the real, non-placeholder fixture files themselves remains an outstanding follow-up — see [docs/IMPLEMENTATION_STATUS.md](IMPLEMENTATION_STATUS.md) for current status.
...
- **Whether/how this adapter is wired into `bcs doctor`** — not decided here, mirroring how every sibling adapter's design left its own CLI wiring as an open question. (The Host Discovery Orchestrator wiring itself is now done — the `filesystem` slot is bound in `bcs.app.main()`; see [docs/HOST_DISCOVERY_ORCHESTRATOR.md § Dependency Injection Strategy](HOST_DISCOVERY_ORCHESTRATOR.md#dependency-injection-strategy---implemented).)
